	...
	$(function () {
	  ...
	  var table = $('.data-table').DataTable({
		  ...
		  columns: [
			  ...
			  {data: 'amenities_id', name:'amenities_id'},
			  {data: 'date', name: 'date'},
            ...
            render: function (data, type, row) {
                if (data == 0) {
                    return '<button class="btn btn-danger toggle-status" data-id="' + row.id + '" data-status="0">Notapproved</button>';
                } else {
                    return '<button class="btn btn-success toggle-status" data-id="' + row.id + '" data-status="1">Approved</button>';
                }
            },
            orderable: false,
            searchable: false
